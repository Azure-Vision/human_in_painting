...
def train(opt):
    ...
    if opt.naive_img != "":
        ...
        naive_img_large = imresize_to_shape(
            naive_img, reals[-1].shape[2:], opt)
    ...
    for scale_num in range(opt.start_scale, opt.train_stages):
        ...
        functions.save_image(
            '{}/real_scale.jpg'.format(opt.outf), reals[scale_num])
        ...
        if opt.fine_tune:
            ...
            d_curr.load_state_dict(torch.load(
                '%s/%d/netD.pth' % (opt.out_, scale_num - 1)))
            generator.init_next_stage()

        writer = SummaryWriter(log_dir=opt.outf)
        fixed_noise, noise_amp, generator, d_curr = train_single_scale(
            d_curr, generator, reals, img_to_augment, naive_img, naive_img_large, fixed_noise, noise_amp, opt, scale_num, opt.train_stages, writer)
...
def train_single_scale(netD, netG, reals, img_to_augment, naive_img, naive_img_large,
                       fixed_noise, noise_amp, opt, depth, total_depth, writer):
        ...
        if depth == 0:
            if opt.train_mode == "harmonization":
                ...
                z_opt = reals[0] + opt.noise_scaling * functions.generate_noise(
                    [opt.nc_im, reals_shapes[depth][2], reals_shapes[depth][3]], device=opt.device).detach()
        else:
    ...
    optimizerD = optim.Adam(
        netD.parameters(), lr=opt.lr_d, betas=(opt.beta1, 0.999))
    ...
    parameter_list = [
        {"params": block.parameters(), "lr": opt.lr_g * (opt.lr_scale **
                                                         (len(netG.body[-opt.train_depth:]) - 1 - idx))}
        for idx, block in enumerate(netG.body[-opt.train_depth:])]

    # add parameters of head and tail to training
    if depth - opt.train_depth < 0:
        parameter_list += [{"params": netG.head.parameters(),
                            "lr": opt.lr_g * (opt.lr_scale ** depth)}]
    parameter_list += [{"params": netG.tail.parameters(), "lr": opt.lr_g}]
    optimizerG = optim.Adam(parameter_list, lr=opt.lr_g,
    ...
    _iter = range(opt.niter)
    total_iter = len(_iter)
    # start training
    # _iter = tqdm(range(opt.niter))
    for iter_i, iter in enumerate(_iter):
        # for iter in _iter:
        # _iter.set_description('stage [{}/{}]:'.format(depth, opt.stop_scale))

        # NOTE: 是training & finetuning的进度条！
        progress = 100 * (iter_i/total_iter/total_depth + depth/total_depth)
        print(progress)
        ############################
        ...
        if iter % 250 == 0 or iter + 1 == opt.niter:
            writer.add_scalar(
                'Loss/train/D/real/{}'.format(j), -errD_real.item(), iter + 1)
            writer.add_scalar('Loss/train/D/fake/{}'.format(j),
                              errD_fake.item(), iter + 1)
            writer.add_scalar(
                'Loss/train/D/gradient_penalty/{}'.format(j), gradient_penalty.item(), iter + 1)
            writer.add_scalar('Loss/train/G/gen', errG.item(), iter + 1)
            writer.add_scalar('Loss/train/G/reconstruction',
                              rec_loss.item(), iter + 1)
            functions.save_image(
                '{}/fake_sample_{}.jpg'.format(opt.outf, iter + 1), fake.detach())
            functions.save_image(
                '{}/reconstruction_{}.jpg'.format(opt.outf, iter + 1), rec.detach())
...
def generate_samples(netG, img_to_augment, naive_img, naive_img_large, aug, opt, depth,
    ...
    with torch.no_grad():
        for idx in range(n):
            ...
            functions.save_image(
                '{}/{}_naive_sample.jpg'.format(dir2save, idx), augmented_image)
            functions.save_image(
                '{}/{}_{}_sample.jpg'.format(dir2save, idx, _name), sample.detach())
            augmented_image = imresize_to_shape(
                augmented_image, sample.shape[2:], opt)
            images.append(augmented_image)
            images.append(sample.detach())

        if opt.fine_tune:
            mask_file_name = '{}_mask{}'.format(
                opt.naive_img[:-4], opt.naive_img[-4:])
            augmented_image = imresize_to_shape(
                naive_img_large, sample.shape[2:], opt)
            if os.path.exists(mask_file_name):
                mask = get_mask(mask_file_name, augmented_image, opt)
                sample_w_mask = (1 - mask) * augmented_image + \
                    mask * sample.detach()
                functions.save_image(
                    '{}/{}_sample_w_mask_{}.jpg'.format(dir2save, _name, iter), sample_w_mask.detach())
                images = torch.cat(
                    [augmented_image, sample.detach(), sample_w_mask], 0)
                grid = make_grid(images, nrow=3, normalize=True)
                writer.add_image('{}_images_{}'.format(
                    _name, depth), grid, iter)
            else:
                print("Warning: no mask with name {} exists for image {}".format(
                ...
                writer.add_image('{}_images_{}'.format(
                    _name, depth), grid, iter)
            functions.save_image(
                '{}/{}_sample_{}.jpg'.format(dir2save, _name, iter), sample.detach())
        else:
            if naive_img is not None:
                noise = []
                for d in range(depth + 1):
                ...
                if os.path.exists(mask_file_name):
                    ...
                    sample_w_mask = (1 - mask) * _naive_img + \
                        mask * sample.detach()
                    functions.save_image(
                        '{}/{}_sample_w_mask_{}.jpg'.format(dir2save, _name, iter), sample_w_mask)
...
def get_mask(mask_file_name, real_img, opt):
    ...
    if mask.shape[3] != real_img.shape[3]:
        mask = imresize_to_shape(
            mask, [real_img.shape[2], real_img.shape[3]], opt)
