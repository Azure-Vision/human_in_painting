...
def generate_samples(netG, reals_shapes, noise_amp, scale_w=1.0, scale_h=1.0, reconstruct=False, n=50, fixed_noise=None, dir2save=None, reals=None, _name=None, real=None, opt=None):
    if reconstruct:
        ...
        if opt.train_mode == "generation" or opt.train_mode == "retarget":
            functions.save_image(
                '{}/reconstruction.jpg'.format(dir2save), reconstruction.detach())
            functions.save_image(
                '{}/real_image.jpg'.format(dir2save), reals[-1].detach())
        elif opt.train_mode == "harmonization" or opt.train_mode == "editing":
            functions.save_image(
                '{}/{}_wo_mask.jpg'.format(dir2save, _name), reconstruction.detach())
    ...
    if scale_w == 1. and scale_h == 1.:
        ...
        reals_shapes = [[r_shape[0], r_shape[1], int(
            r_shape[2]*scale_h), int(r_shape[3]*scale_w)] for r_shape in reals_shapes]
        dir2save_parent = os.path.join(
            dir2save, "random_samples_scale_h_{}_scale_w_{}".format(scale_h, scale_w))

    make_dir(dir2save_parent)

    for idx in range(n):
        noise = functions.sample_random_noise(
            opt.train_stages - 1, reals_shapes, opt)
        sample = netG(noise, reals_shapes, noise_amp)
        functions.save_image(
            '{}/gen_sample_{}.jpg'.format(dir2save_parent, idx), sample.detach())
...
def main(opt):
    ...
    if opt.train_mode == "generation" or opt.train_mode == "retarget":
        ...
        with torch.no_grad():
            ...
            rs0 = generate_samples(
                netG, reals_shapes, noise_amp, n=opt.num_samples)
    ...
    if opt.train_mode == "harmonization" or opt.train_mode == "editing":
        ...
        out = generate_samples(netG, reals_shapes, noise_amp, reconstruct=True,
                               fixed_noise=fixed_noise, dir2save=dir2save, reals=reals, _name=_name, real=real, opt=opt)
        ...
        if os.path.exists(mask_file_name):
            ...
            if mask.shape[3] != out.shape[3]:
                mask = imresize_to_shape(
                    mask, [out.shape[2], out.shape[3]], opt)
            mask = functions.dilate_mask(mask, opt)
            out = (1 - mask) * reals[-1] + mask * out
            functions.save_image(
                '{}/{}_w_mask.jpg'.format(dir2save, _name), out.detach())
        else:
